        ...
        <ul v-for="order1 in product" :key="order1.id" class="msg-ul">
        	<li>
        		...
        		<div class="total">
        			...
        			<div class="btn">
        				<el-button type="text" @click='payOrder(order1.date_id)'>评价</el-button>
        			</div>
        		</div>
        	</li>
        </ul>
...
export default {
	...
	let sql1='SELECT * FROM book_date where username ="' + user + '" and order_date=3';
	axios.get('http://132.232.114.138/php/datainfo.php?sql='+sql1).then(res=>{
    	...
    	for (let j in this.product) {
    		let sql2='select * from book_order where order_state=3 and date_id="' + this.product[j].date_id + '" and user_name="'+user+'"';
  ...
  methods:{
    ...
    payOrder(date_id){
        this.$prompt('请输入您对此书的评论', '提示', {
            ...
            let user = sessionStorage.getItem('user');
            let com_time = new Date().toLocaleDateString();
            let user_img='head.jpg';
            let sql = 'INSERT INTO book_comment (date_id,user_name,comment_time,comment_message,user_img) values("'+date_id+'","'+user+'","'+com_time+'","'+value+'","'+user_img+'")'
                axios.get('/api/php/dataupdate.php?sql='+sql).then((res)=>{
                })
            }).catch(() => {
            this.$message({
                type: 'info',
