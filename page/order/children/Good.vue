...
export default {
    ...
    created(){
	 ...
	 let sql1='SELECT * FROM book_date where username ="' + user + '" and order_date=1';
	 axios.get('http://132.232.114.138/php/datainfo.php?sql='+sql1).then(res=>{
    	...
    	for (let j in this.product) {
    		let sql2='select * from book_order where order_state=1 and date_id="' + this.product[j].date_id + '" and user_name="'+user+'"';
  ...
  methods:{
    ...
    delOrder(date_id) {
    	this.$confirm('即将退款, 是否继续?', '提示', {
			...
			let sql3='UPDATE book_order SET order_state=3 WHERE date_id ='+ date_id;
			axios.get('http://132.232.114.138/php/datainfoa.php?sql='+sql3).then(res=>{
    		});
			let sql4='UPDATE book_date SET order_date=3 WHERE date_id ='+ date_id;
