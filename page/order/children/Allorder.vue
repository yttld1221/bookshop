...
<script>
	...
	export default {
		...
		created() {
			...
			let sql1='SELECT * FROM book_date where username ="' + user + '"';
			axios.get('http://132.232.114.138/php/datainfo.php?sql='+sql1).then(res=>{
				...
				for (let j in this.product) {
					let sql2='select * from book_order where user_name ="' + user + '" and date_id="' + this.product[j].date_id + '" and user_name="'+user+'"';
