        ...
        <ul v-for="order1 in product" :key="order1.id" class="msg-ul">
        	<li>
        		...
        		<div class="total">
        			...
        			<div class="btn">
        				<el-button type="text" @click='delOrder(order1.date_id)'>删除订单</el-button>
								 <el-button type="text" @click="payOrder(order1.date_id)">付款</el-button>
        			</div>
        		</div>
        	</li>
        </ul>
...
export default {
	...
	let sql1='SELECT * FROM book_date where username ="' + user + '" and order_date=0';
	axios.get('http://132.232.114.138/php/datainfo.php?sql='+sql1).then(res=>{
    	...
    	for (let j in this.product) {
    		let sql2='select * from book_order where order_state=0 and date_id="' + this.product[j].date_id + '" and user_name="'+user+'"';
  ...
  methods:{
    ...
    delOrder(date_id) {
    	...
    	this.$confirm('即将删除订单, 是否继续?', '提示', {
			...
			let sql3='DELETE FROM book_order WHERE date_id ='+ date_id;
			axios.get('http://132.232.114.138/php/datainfoa.php?sql='+sql3).then(res=>{
    		});
			
			let sql4='DELETE FROM book_date WHERE date_id ='+ date_id;
    ...
    payOrder(date_id){
      ...
      this.$confirm('即将付款, 是否继续?', '提示', {
				...
				let sql5='UPDATE book_order SET order_state=1 WHERE date_id ='+ date_id;
				axios.get('http://132.232.114.138/php/datainfoa.php?sql='+sql5).then(res=>{
                });
				let sql6='UPDATE book_date SET order_date=1 WHERE date_id ='+ date_id;
